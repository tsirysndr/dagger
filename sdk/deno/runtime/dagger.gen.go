...
import (
	...
	"deno-sdk/internal/querybuilder"
)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "DenoSdk":
		...
		case "ModuleRuntime":
			...
			return (*DenoSdk).ModuleRuntime(&parent, modSource, opts), nil
		case "Codegen":
			var err error
			var parent DenoSdk
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*DenoSdk).Codegen(&parent, modSource, opts), nil
		case "Base":
			var err error
			var parent DenoSdk
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*DenoSdk).Base(&parent, platform), nil
		default:
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("DenoSdk").
					WithFunction(
