...
import (
	...
	"dagger.io/dagger/runtime/internal/querybuilder"
)
	...
	case "GoSdk":
		...
		case "ModuleRuntime":
			...
			if inputArgs["subPath"] != nil {
				err = json.Unmarshal([]byte(inputArgs["subPath"]), &opts.SubPath)
				if err != nil {
					fmt.Println(err.Error())
					os.Exit(2)
				}
			...
			if inputArgs["platform"] != nil {
				err = json.Unmarshal([]byte(inputArgs["platform"]), &opts.Platform)
				if err != nil {
					fmt.Println(err.Error())
					os.Exit(2)
				}
			}
			return (*GoSdk).ModuleRuntime(&parent, modSource, opts), nil
		case "Codegen":
			var err error
			var parent GoSdk
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*GoSdk).Codegen(&parent, modSource, opts), nil
		case "Base":
			var err error
			var parent GoSdk
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*GoSdk).Base(&parent, platform), nil
		case "CodegenBin":
			var err error
			var parent GoSdk
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*GoSdk).CodegenBin(&parent, platform), nil
		default:
